...
def collect_du_result(cmd = 'sh du_awk.sh',forensics_type='network',start_date='18480101',end_date='18481229',out_put='forensics.txt'):
    ...
    """
    通过shell脚本获取当前目录的文件和文件夹大小
    """
    if forensics_type in ['network','endpoint','email']:
        cmd = cmd + ' -f ' + forensics_type
    else:
        print "Forensics type error!!!"
    if len(start_date)==8 and start_date>'19000101':
        cmd = cmd + ' -s ' + start_date
    else:
        print "Start date error!!!"
    if len(end_date)==8 and end_date > '19000101':
        cmd = cmd + ' -e ' + end_date
    else:
        print "End data error!!!"
    out_put_file = '/tmp/' + forensics_type + '_' + out_put
    cmd = cmd + ' -o ' + out_put_file
    os.system(cmd)
    var_available_size_cmd = "df -k /var | awk 'END {print $4}'"
    var_available_size = os.system(var_available_size_cmd)
    return var_available_size
...
def filter_great_dir(du_result,great_size='100M',rate=1.0):
    ...
    while i<len(du_result):
        ...
        if get_int_size(du_result[i].split(' ')[0])>=(great_size_int*rate):
            ...
            #du_result.pop(i)
        i = i + 1
    return great_dir_list,list(set(du_result).difference(set(great_dir_list)))

 
def tar_and_zip(target_dirs,keyword='forensics',is_same_dir=True,root_dir='/var/skyguard/sps/forensics/incident/network/'):
    """
    打包和压缩文件夹列表
    ...
    print 'tar target file: ' + full_tar_file_name
